...
is.empty = function(value) {
  ...
  if (isArrayLike(value) && Reflect.has(val, 'size') && val.size === 0) {
    return true
  }
...
/**
 * is.falsy
 * Test if `value` is falsy (undefined, null, false, 0, -0, '', NaN).
 *
 * @param {*} value to test
 * @param {*} host host to test with
 * @return {Boolean} true if `value` is falsy, false otherwise
 * @api public
 */

is.falsy = function isFalsy(value) {
  return !!(
    value === `` ||
    value === 0 ||
    value === -0 ||
...
/**
 * is.truthy
 * Test if `value` is truthy, or not one of the values specified in is.falsy().
 *
 * @param {*} value to test
 * @param {*} host host to test with
 * @return {Boolean} true if `value` is falsy, false otherwise
...
is.truthy = function isTruthy(value) {
...
/**
 * is.plainObject
 * Test if `value` is an plain, vanilla, object.
 *
 * @param {*} value value to test
